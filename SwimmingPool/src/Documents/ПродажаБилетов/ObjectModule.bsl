...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если БаллыКСписанию <> 0 Тогда
		...
		Если БаллыКСписанию > СуммаПродажи Тогда
			...
			Сообщение.Текст = "Количество списываемых баллов превышает сумму продажи";
		...
		Если Не ЗначениеЗаполнено(Клиент) Тогда
			Отказ = Истина;
			Сообщение.Текст = СтрШаблон("Балы нельзя списать, если билет не персонализирован");
			Сообщение.УстановитьДанные(ЭтотОбъект);
			Сообщение.Поле = "БаллыКСписанию";
			Сообщение.Сообщить();
		КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ПродажаБилетовПозицииПродажи.Номенклатура.ВидБассейна КАК ВидБассейна,
	...
	Пока Выборка.Следующий() Цикл
		...
		Движение.ВидыБассейнов = Выборка.ВидБассейна;
		...
		Движение.ВидыБассейна = Выборка.ВидБассейна;
